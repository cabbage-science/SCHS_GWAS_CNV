...
bed_data <- read.table("../data/binned_hg19_genome_100kb_10kb.bed", header = FALSE, col.names = column_names, stringsAsFactors = FALSE) %>%
...
#cnv_raw <- data.frame(fread("../data/CNV_batch_1_5_all_indivs_blacklist_filtered.txt", header = TRUE, col.names = CNV_colnames))
#cnv_raw <- data.frame(fread("../data/CNV_batch_1_5_all_indivs_blacklist_filtered_20kb_5nsnp.txt", header = TRUE, col.names = CNV_colnames))
#cnv_raw <- data.frame(fread("../data/CNV_batch_1_5_all_indivs_blacklist_filtered_10kb_10nsnp.txt", header = TRUE, col.names = CNV_colnames))
cnv_raw <- data.frame(fread("../data/CNV_batch_1_5_all_indivs_blacklist_filtered_10kb_5nsnp.txt", header = TRUE, col.names = CNV_colnames))
cnv_raw <- cnv_raw[79391:150271,]
...
count_cnvs_per_sliding_window <- function(cnv_df,bed_df,overlap=10,window_size=200000) {
  ...
  # Also reconfiguring all samples names to integer format to match 
  # sample IDs of (reconfigured) Phenotype_GSA_filtered file later on during association testing
        ...
        for(m in 1:nrow(temp_cnv_df_by_chr)) {
          ...
          if(nrow(temp_cnv_df_by_chr) > 0) {
            ...
            if(temp_cnv_df_by_chr[m,"start"] >= temp_bed_df[k,"start"] & temp_cnv_df_by_chr[m,"end"] <= temp_bed_df[k,"end"]) {
              ...
              # If the CNV is larger than the window itself
  ...
  # Reconfiguring all samples names to integer format to match 
  # sample IDs of (reconfigured) Phenotype_GSA_filtered file later on during association testing
  output_df <- output_df %>%
    mutate(sample_ID = str_replace(sample_ID, "225_", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "SCHS_", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "_01_GlobalScreening_iA1_1_GlobalScreening", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "_01_GlobalScreening_iA1_2_GlobalScreening", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "_03_GlobalScreening_iA1_1_GlobalScreening", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "_04_GlobalScreening_iA1_1_GlobalScreening", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "_02_GlobalScreening_iA1_1_GlobalScreening", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "startsnp", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "startsn", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "starts", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "start", "")) %>%
    mutate(sample_ID = str_replace(sample_ID, "star", "")) %>%
    mutate(sample_ID = as.integer(str_trim(sample_ID)))
  
  # Removing rows with NA values in sample ID, due to reconfiguration step earlier
  output_df <- output_df[!is.na(output_df$sample_ID),] %>% 
    # Sorting for detection of duplicated sample IDs later on
    arrange(sample_ID)
  
  # Detect and remove rows with duplicated sample_IDs
  geno_duplicated <- which(duplicated(output_df$sample_ID))
  # Creating vector of sample_IDs with duplications
  geno_duplicated_ids <- output_df[geno_duplicated, "sample_ID"]
  # Remove ALL rows that are duplicated due to possibility that the sample with the same
  # ID could have different CNV genotype.
  output_df <- output_df %>%
    filter(!sample_ID %in% geno_duplicated_ids)
...
CNV_matrix <- count_cnvs_per_sliding_window(cnv_raw, bed_data, window_size = 100000)

# Writing a txt file for the matrix of CNV counts per individual per window
# Remove/add the comment hashtag where appropriate.
#write.table(CNV_matrix, "../results/CNV_matrix_indivs_windows_18Ksamples.txt", quote = FALSE, sep = "\t", row.names = FALSE)
#write.table(CNV_matrix, "../results/CNV_matrix_indivs_windows_18Ksamples_100kbWindow_10nsnp.txt", quote = FALSE, sep = "\t", row.names = FALSE)
#write.table(CNV_matrix, "../results/CNV_matrix_indivs_windows_18Ksamples_200kbWindow_5nsnp.txt", quote = FALSE, sep = "\t", row.names = FALSE)
write.table(CNV_matrix, "../results/CNV_matrix_indivs_windows_18Ksamples_100kbWindow_5nsnp_2.txt", quote = FALSE, sep = "\t", row.names = FALSE)
